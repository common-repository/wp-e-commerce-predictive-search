...
class WPSC_Predictive_Search_Meta
{
	public static function create_custombox() {
		...
		$hide_item_from_result_text = ' <span style="float:right;" class="a3_ecommerce_search_exclude_item"><label><input style="position: relative; top: 2px;" type="checkbox" '.$check.' value="1" name="_ecommerce_search_exclude_item" /> '.__('Hide from Predictive Search results.', 'wp-e-commerce-predictive-search' ).'</label></span>';
		
		add_meta_box( 'wpec_predictive_search_metabox', __('Predictive Search Meta', 'wp-e-commerce-predictive-search' ).$hide_item_from_result_text , array('WPSC_Predictive_Search_Meta','data_metabox'), 'post', 'normal', 'high' );
		
		add_meta_box( 'wpec_predictive_search_metabox', __('Predictive Search Meta', 'wp-e-commerce-predictive-search' ).$hide_item_from_result_text , array('WPSC_Predictive_Search_Meta','data_metabox'), 'page', 'normal', 'high' );
		add_meta_box( 'wpec_predictive_search_metabox', __('Predictive Search Meta', 'wp-e-commerce-predictive-search' ).$hide_item_from_result_text , array('WPSC_Predictive_Search_Meta','data_metabox'), 'wpsc-product', 'normal', 'high' );
	}
	...
	?>
    	<table class="form-table" cellspacing="0">
        	<tr valign="top">
				<th scope="rpw" class="titledesc"><label for="_predictive_search_focuskw"><?php _e('Focus Keywords', 'wp-e-commerce-predictive-search' ); ?></label></th>
	...
	public static function save_custombox($post_id) {
		...
		if(in_array($post_type, array('post', 'page', 'wpsc-product') ) && isset($_REQUEST['_predictive_search_focuskw']) && $post_status != false  && $post_status != 'inherit') {
			extract($_REQUEST);
